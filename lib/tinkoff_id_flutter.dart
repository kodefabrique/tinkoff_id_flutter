...
class TinkoffIdFlutter {
  ...
  Future<bool> isTinkoffAuthAvailable() => TinkoffIdFlutterPlatform.instance.isTinkoffAuthAvailable();

  ///Launches the Tinkoff Bank application, where the user enters authorization data. No exceptions throws
  Future startTinkoffAuth(String redirectUri) => TinkoffIdFlutterPlatform.instance.startTinkoffAuth(redirectUri);
  ...
  Future<TokenPayload> getTokenPayload(String incomingUri) =>
      TinkoffIdFlutterPlatform.instance.getTokenPayload(incomingUri).then((value) => TokenPayload.fromJson(value));

  ///Gets new tokens by the refresh token. Throws [PlatformException]
  Future<TokenPayload> updateToken(String refreshToken) {
    return TinkoffIdFlutterPlatform.instance.updateToken(refreshToken).then((value) => TokenPayload.fromJson(value));
  }
